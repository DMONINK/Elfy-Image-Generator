...
async def handle_message(message: Message, ai_service, storage_manager) -> None:
    ...
    try:
        async with message.channel.typing():
            ...
            # Generate response (may be text-only or text + image)
            ...
            # If we got image bytes back, send the image as a file
            if image_bytes is not None:
                ...
                # Regular text response — split if needed
                if response_text:
                    await split_and_send_messages(message, response_text, MAX_MESSAGE_LENGTH)

            # Save to persistent storage (only for text chat, not image gen)
